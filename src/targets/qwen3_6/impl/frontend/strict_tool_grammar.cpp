...
Json parse_parameters(const ToolArgumentTypeContracts::Tool& tool) {
...
std::string strict_parameter_slots(RuleSet& rules, const ToolArgumentTypeContracts::Tool& tool,
...
std::string strict_call_body(RuleSet& rules, const ToolArgumentTypeContracts::Tool& tool,
                             std::size_t tool_index) {
...
std::string nonstrict_call_body(const ToolArgumentTypeContracts::Tool& tool) {
    std::string body = quote_literal(kToolOpen) + " " + std::string(kWsRule) + " " +
                       quote_literal("<function=" + tool.name + ">") + " " +
...
std::string compose_output_grammar(const ToolArgumentTypeContracts* contract,
    ...
    if (has_tools) {
        ...
        std::size_t index = 0;
        for (const ToolArgumentTypeContracts::Tool& tool : contract->tools) {
